...
Render one practice payload into a self-contained HTML page.
  ...
  python tools/build_static.py payloads/demo-kilburn-park.json               # -> dist/<ods>.html (placeholders shown)
  python tools/build_static.py payloads/demo-kilburn-park.json --demo        # real data only (conference fallback)
...
def main():
    ...
    ap.add_argument("--demo", action="store_true", help="default to real-data-only mode")
    ...
    body = tpl.replace(tag + "__PAYLOAD__", (tag.replace('">', f'" data-mode="{"demo" if a.demo else "mark"}">')) + blob)
    ...
    out = pathlib.Path(a.out) if a.out else ROOT / "dist" / f"{data['practice']['ods_code']}{'-demo' if a.demo else ''}.html"
